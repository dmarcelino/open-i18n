...
class SitePress{
    ...
    function __construct(){
        ...
        if(is_admin() && $pagenow == 'index.php'){
            ...
            if(trim(get_option('permalink_structure'), '/') == '%postname%'){
				// TODO BUGFIX: why is it a dangerous permalink structure? discover and fix whatever issue there is
    ...
    function administration_menu(){
        ...
        if (1 < count($this->get_active_languages())) {
            ...
            add_menu_page(__(WP_I18N_DISPLAY_NAME,'sitepress'), __(WP_I18N_DISPLAY_NAME,'sitepress'), 'manage_options', basename(ICL_PLUGIN_PATH).'/menu/translation-management.php',null, ICL_PLUGIN_URL . '/res/img/icon16.png');
            ...
            add_menu_page(__(WP_I18N_DISPLAY_NAME,'sitepress'), __(WP_I18N_DISPLAY_NAME,'sitepress'), 'manage_options', basename(ICL_PLUGIN_PATH).'/menu/languages.php',null, ICL_PLUGIN_URL . '/res/img/icon16.png');
            add_submenu_page(basename(ICL_PLUGIN_PATH).'/menu/' . $main_page, __('Languages','sitepress'), __('Languages','sitepress'),
                 'manage_options', basename(ICL_PLUGIN_PATH).'/menu/languages.php');
        }
        ...
        addLoadEvent(function(){
            jQuery.ajax({type: "POST",url: icl_ajx_url,data: "icl_ajx_action=health_check", error: function(msg){
                    ...
                    jQuery('.wrap').prepend('<div class="error"><p><?php 
                        echo icl_js_escape(sprintf(__("%s can't run normally. There is an installation or server configuration problem. %sShow details%s",'sitepress'), WP_I18N_DISPLAY_NAME, 
    ...
    function noscript_notice(){
        ?><noscript><div class="error"><?php echo sprintf(__('%s admin screens require JavaScript in order to display. JavaScript is currently off in your browser.', 'sitepress'),WP_I18N_DISPLAY_NAME) ?></div></noscript><?php
    }
    ...
    function show_user_options(){
        ...
        ?>
        <a name="wpml"></a>
        <h3><?php _e('Language settings','sitepress'); ?></h3>
    ...
    function help_admin_notice(){  
        ...
        <?php _e('Would you like to see a quick overview of the plugin features?', 'sitepress'); ?>
        </p>
        <p>
    ...
    function dashboard_widget_setup(){
        if (current_user_can('manage_options')) {
            ...
            wp_add_dashboard_widget($icl_dashboard_widget_id, sprintf(__('Multi-language | %s %s', 'sitepress'),WP_I18N_DISPLAY_NAME, ICL_SITEPRESS_VERSION), array($this, 'dashboard_widget'), null);
            if($install){
    ...
    function warn_permalink_structure(){        
        ...
        <div class="error message fade">
            <p><?php _e("This permalink format can cause problems with translations.", 
                'sitepress'); ?></p>
        </div>
        <?php
        else:
        ...
        <div class="icl_form_errors" style="width: 98%;">
            <p><?php _e("The current permalink format can cause problems with translations."); ?></p>
        </div>        
        <?php
        endif;
    }
