...
class SitePressLanguageSwitcher {
	...
	function init(){
		...
		if (is_admin()) {
			...
			icl_register_string(ICL_PLUGIN_SHORT_NAME, 'Text for alternative languages for posts', $this->settings['icl_post_availability_text']);
	...
	function post_availability($content){
        ...
        if(is_singular()){
            ...
            if(1 < count($languages)){
				...
				$out = '<p>' . sprintf(icl_t(ICL_PLUGIN_SHORT_NAME, 'Text for alternative languages for posts', $this->settings['icl_post_availability_text']), $out) . '</p>';
            }
        }
...
// language switcher functions
	function language_selector_widget($args){            
		...
		if ($sitepress_settings['icl_widget_title_show']) {
			...
			echo icl_t(ICL_PLUGIN_SHORT_NAME, 'Widget title', __('Languages','sitepress'));
