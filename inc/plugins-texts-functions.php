...
function icl_pt_get_texts(){
    ...
    foreach($active_plugins as $ap){    
        ...
        if(isset($rs[$ap])){
            ...
            $fields_list = sprintf(__('%s doesn\'t know how to translate this plugin. If it has texts that require translation, contact us: %s', 'sitepress'), WP_I18N_DISPLAY_NAME, '<a href="' . WP_I18N_URI_SUPPORT . '">' . WP_I18N_URI_SUPPORT . '</a>');
            $active = 0;
        }                
        //exception for WPML
        if($plugin_name_short==ICL_PLUGIN_FOLDER) $plugin_name_short = 'WP_i18n';
        $texts[] = array(
            'active' => $active,
