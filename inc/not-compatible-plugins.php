...
if(!empty($icl_ncp_plugins)){
    ...
    function icl_incomp_plugins_warn(){
        ...
        echo sprintf(__('%s cannot be activated together with these older plugins:', 'sitepress'),WP_I18N_DISPLAY_NAME);
        echo '<ul style="list-style:disc;margin:20px;">';
        foreach($icl_ncp_plugins as $incp){
            echo '<li>'.$incp.'</li>';
        }
        echo '</ul>';
        echo sprintf(__('%s will be deactivated', 'sitepress'),WP_I18N_DISPLAY_NAME);
