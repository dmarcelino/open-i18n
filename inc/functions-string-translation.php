    ...
    if(!$recursion && !$double_scan){
        ...
        $scan_stats .= __('Done scanning files', 'sitepress') . PHP_EOL;
            ...
            $scan_stats_all = __('= Your theme was scanned for texts =', 'sitepress') . '<br />' . 
                          ...
                          sprintf(__('%s found %s strings. They were added to the string translation table.','sitepress'),ICL_PLUGIN_DISPLAY_NAME,count($__icl_registered_strings)) . 
    ...
    if(!$recursion){
        ...
        */
        ...
        if(count($__icl_registered_strings)){
            $scan_stats .=  sprintf(__('%s found %s strings. They were added to the string translation table.','sitepress'),ICL_PLUGIN_DISPLAY_NAME,count($__icl_registered_strings)) . "\n";
        }else{
...
function icl_st_generate_po_file($strings, $potonly = false){
    ...
    $po = "";
    $po .= '# This file was generated by ' . WP_I18N_DISPLAY_NAME . PHP_EOL;
    $po .= '# ' . WP_I18N_DISPLAY_NAME . ' is a WordPress plugin that can turn any WordPress or WordPressMU site into a full featured multilingual content management system.' . PHP_EOL;    
    $po .= '# ' . WP_I18N_URI . PHP_EOL;
    $po .= 'msgid ""' . PHP_EOL;
    $po .= 'msgstr ""' . PHP_EOL;
